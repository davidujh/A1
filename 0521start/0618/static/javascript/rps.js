...
$(function(){
    ...
    if(storageData){//licalStorage에 저장된 값
        record=storageData
        console.log(record);
    //            유져          컴            결과
    }else{
...
function userSelect(){
    ...
    // userImg 클래스들을 배열로 가져오고 그중에서 몇번째 인덱스를 클릭했나 ?
    $(this).css('border','1px solid white'); // 내가 클릭한 가위바위보 이미지 표시

    clearInterval(comHd);// 컴퓨터의 가위바위보 이미지 변경하는거 멈추기 (clearInterval)
    ...
    },1000 ); // 지정된 시간 이후에 한번 실행


     // 다시 컴퓨터 가위바위보 이미지 변경되게 (setInterval)
...
function outcome(u , c){ // u는 유저가위바위보 ,c는 컴 가위바위보
    // 0-가위 1-바위 2-보
    var result = "승"
    var minus = u-c; //
    switch(minus){
...
function localSave(){
    ...
    // JSON.stringify() JSON이라는 문자열로 면환시켜준다/
    // 숫자 -> 문자열 10+""
